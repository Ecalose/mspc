...
Get-ChildItem -Recurse -Path .\bucket | ForEach-Object -Process {
    ...
    if ($content -match 'gh-proxy.com') {
        # 如果包含，则输出消息并跳出循环
        Write-Output "文本中包含 'gh-proxy.com'，进行下一个。"
        return
    }
    # GitHub Releases
    ...
    # OBS Studio
    (Get-Content $_.FullName) -replace 'cdn-fastly\.obsproject\.com/downloads/OBS-Studio-(.+)-Windows\.zip', 'gh-proxy.com/https://github.com/obsproject/obs-studio/releases/download/$1/OBS-Studio-$1-Windows.zip' | Set-Content -Path $_.FullName
    # Or
