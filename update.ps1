...
Get-ChildItem -Recurse -Path .\bucket | ForEach-Object -Process {
    # GitHub Releases
    (Get-Content $_.FullName) -replace '(github\.com/.+/releases/.*download)', '${mirrorUrl}/https://$1' | Set-Content -Path $_.FullName

    # GitHub Archive
    (Get-Content $_.FullName) -replace '(github\.com/.+/archive/)', '${mirrorUrl}/https://$1' | Set-Content -Path $_.FullName

    # GitHub Raw
    (Get-Content $_.FullName) -replace '(raw\.githubusercontent\.com)', '${mirrorUrl}/https://$1' | Set-Content -Path $_.FullName
    (Get-Content $_.FullName) -replace '(github\.com/.+/raw/)', '${mirrorUrl}/https://$1'          | Set-Content -Path $_.FullName
    ...
    # DBeaver
    (Get-Content $_.FullName) -replace 'dbeaver\.io/files', '${mirrorUrl}/https://github.com/dbeaver/dbeaver/releases/download' | Set-Content -Path $_.FullName
    # Or
    ...
    # OBS Studio
    (Get-Content $_.FullName) -replace 'cdn-fastly\.obsproject\.com/downloads/OBS-Studio-(.+)-Windows\.zip', '${mirrorUrl}/https://github.com/obsproject/obs-studio/releases/download/$1/OBS-Studio-$1-Windows.zip' | Set-Content -Path $_.FullName
    # Or
    ...
    # OBS Studio 2.7
    (Get-Content $_.FullName) -replace 'cdn-fastly\.obsproject\.com/downloads/OBS-Studio-(.+)-Full', '${mirrorUrl}/https://github.com/obsproject/obs-studio/releases/download/$1/OBS-Studio-$1-Full' | Set-Content -Path $_.FullName
    ...
    # Strawberry
    (Get-Content $_.FullName) -replace 'files\.jkvinge\.net/packages/strawberry/StrawberrySetup-(.+)-mingw-x', '${mirrorUrl}/https://github.com/strawberrymusicplayer/strawberry/releases/download/$1/StrawberrySetup-$1-mingw-x' | Set-Content -Path $_.FullName
    ...
    # FastCopy
    (Get-Content $_.FullName) -replace 'fastcopy\.jp/archive', '${mirrorUrl}/https://raw.githubusercontent.com/FastCopyLab/FastCopyDist2/main' | Set-Content -Path $_.FullName
